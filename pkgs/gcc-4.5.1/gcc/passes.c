...
/* HACK: Niranjan: Introduced to selectively execute the passes 
 *
 * Pointer saves the node of the pass in the pass list from where
 * we want to execute all forward passes.
 *
 * Execution of passes is done by plugin.
 */
struct opt_pass **save_pass_ptr ;
struct opt_pass ** get_save_pass_ptr(void);
...
/*
 * Niranjan: Function to get access to save_pass_ptr
 */
struct opt_pass ** get_save_pass_ptr(void)
{
		return save_pass_ptr;
}
  ...
  if (pass->static_pass_number != -1)
      ...
      if (dump_file && current_function_decl)
	{
	  ...
	  aname = (IDENTIFIER_POINTER
		...
		/* 
		 * Added by Niranjan: to separate out the old dump and new dump of the same
		 * phase
		 */
		/*fprintf(dump_file, "\n;;----------------------------------------------------------------------");
		fprintf(dump_file, "\n;;This is new dump of the same phase; may be new iteration of the pass");
		fprintf(dump_file, "\n;;----------------------------------------------------------------------");*/
	  fprintf (dump_file, "\n;; Function %s (%s)%s\n\n", dname, aname,
