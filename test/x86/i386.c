		   ...
		   enum x86_64_reg_class classes[MAX_CLASSES], int bit_offset)
{
  ...
  switch (mode)
	...
	return 0;
      ...
      gcc_assert (GET_MODE_CLASS_EQ_MODE_INT(GET_MODE_INNER (mode)));
...
ix86_decompose_address (rtx addr, struct ix86_address *out)
{
  ...
  if (TARGET_K6 && cfun && optimize_function_for_speed_p (cfun)
      ...
      && REG_P (base_reg)
      && REGNO_REG_CLASS_EQ_SIREG(REGNO (base_reg)))
    disp = const0_rtx;
...
print_operand (FILE *file, rtx x, int code)
{
  if (code)
	...
	case 'z':
	  ...
	  if (GET_MODE_CLASS_MODE_FLOAT(GET_MODE (x)))
	    warning
	...
	case 'Z':
	  ...
	  if (GET_MODE_CLASS_MODE_INT(GET_MODE (x)))
	    {
	      switch (GET_MODE_SIZE (GET_MODE (x)))
		{
		case 2:
	  ...
	  else if (GET_MODE_CLASS_MODE_FLOAT(GET_MODE (x)))
	    {
...
output_387_binary_op (rtx insn, rtx *operands)
{
  ...
  switch (GET_CODE (operands[3]))
    {
    case PLUS:
      if (GET_MODE_CLASS_EQ_MODE_INT(GET_MODE (operands[1]))
	  || GET_MODE_CLASS_EQ_MODE_INT(GET_MODE (operands[2])))
	p = "fiadd";
      else
	p = "fadd";
      ssep = "vadd";
    ...
    case DIV:
      if (GET_MODE_CLASS_EQ_MODE_INT(GET_MODE (operands[1]))
	  || GET_MODE_CLASS_EQ_MODE_INT(GET_MODE (operands[2])) == MODE_INT)
	p = "fidiv";
      else
	p = "fdiv";
      ssep = "vdiv";
...
output_fp_compare (rtx insn, rtx *operands, int eflags_p, int unordered_p)
{
  ...
  if (STACK_REG_P (cmp_op1)
	...
	NULL
      ...
      mask  = eflags_p << 3;
      mask |= (GET_MODE_CLASS_EQ_MODE_INT(GET_MODE (cmp_op1))) << 2;
      mask |= unordered_p << 1;
      mask |= stack_top_dies;
...
ix86_expand_vector_move_misalign (enum machine_mode mode, rtx operands[])
{
  ...
  if (MEM_P (op1))
      ...
      if (optimize_insn_for_size_p ())
	{
	 ...
	 integer type.  */
      if (TARGET_SSE2 && GET_MODE_CLASS_EQ_MODE_VECTOR_INT(mode))
	{
	  op0 = gen_lowpart (V16QImode, op0);
	  op1 = gen_lowpart (V16QImode, op1);
...
ix86_cc_modes_compatible (enum machine_mode m1, enum machine_mode m2)
{
  ...
  if (!GET_MODE_CLASS_EQ_MODE_CC(m1) || !GET_MODE_CLASS_MODE_CC(m2))
    return VOIDmode;
...
ix86_expand_compare (enum rtx_code code)
{
  ...
  if (GET_MODE_CLASS_EQ_MODE_CC(GET_MODE (ix86_compare_op0)))
  ...
  switch (GET_MODE (ix86_compare_op0))
    ...
    default:
      /* If we have already emitted a compare insn, go straight to simple.
         ...
         is non NULL.  */
      gcc_assert (GET_MODE_CLASS_EQ_MODE_CC(GET_MODE (ix86_compare_op0)));
      goto simple;
    }
}
...
ix86_expand_int_movcc (rtx operands[])
{
  ...
  if ((mode != HImode || TARGET_FAST_PREFIX)
      ...
      if (diff < 0)
	{
      ...
      if (GET_MODE_CLASS_EQ_MODE_INT(GET_MODE (ix86_compare_op0))
...
ix86_hard_regno_mode_ok (int regno, enum machine_mode mode)
{
  ...
  if (CC_REGNO_P (regno))
    return GET_MODE_CLASS_EQ_MODE_CC(mode);
  if (GET_MODE_CLASS_EQ_MODE_CC(mode)
      || GET_MODE_CLASS_EQ_MODE_RANDOM(mode)
...
ix86_rtx_costs (rtx x, int code, int outer_code_i, int *total, bool speed)
{
  ...
  switch (code)
	...
	*total = cost->divide[MODE_INDEX (mode)];
    ...
    case PLUS:
      if (GET_MODE_CLASS_EQ_MODE_INT(mode)
