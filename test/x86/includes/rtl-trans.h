...
#define GET_RTX_CLASS_EQ_RTX_UNARY(CODE) \
	((CODE) == NEG || (CODE) == NOT || \
	 ((CODE) >= SIGN_EXTEND && (CODE) <= PARITY) || \
	 ((CODE) == VEC_DUPLICATE) || \
	 ((CODE) >= SS_NEG && (CODE) <= SS_ABS) || \
	 ((CODE) >= SS_TRUNCATE && (CODE) <= US_TRUNCATE))

#define GET_RTX_CLASS_EQ_RTX_COMM_ARITH(CODE) \
	((CODE) == PLUS || ((CODE) >= MULT && (CODE) <= US_MULT) || \
	 ((CODE) >= AND && (CODE) <= XOR) || \
	 ((CODE) >= SMIN && (CODE) <= UMAX) || \
	 ((CODE) >= SS_PLUS && (CODE) <= US_PLUS)) 

#define GET_RTX_CLASS_EQ_RTX_BIN_ARITH(CODE) \
	((CODE) == COMPARE || (CODE) == MINUS || \
	 ((CODE) >= DIV && (CODE) <= UMOD) || \
	 ((CODE) >= ASHIFT && (CODE) <= ROTATERT) || \
	 ((CODE) >= VEC_SELECT && (CODE) <= VEC_CONCAT) || \
	 (CODE) == SS_MINUS || ((CODE) >= SS_ASHIFT && (CODE) <= US_MINUS))

#define GET_RTX_CLASS_EQ_RTX_CONST_OBJ(CODE) \
	(((CODE) >= CONST_INT && (CODE) <= CONST_VECTOR) || \
	 ((CODE) == CONST) || ((CODE) >= LABEL_REF && (CODE) <= SYMBOL_REF) || \
	 ((CODE) == HIGH))

#define GET_RTX_CLASS_EQ_RTX_AUTOINC(CODE) \
	((CODE) >= PRE_DEC && (CODE) <= POST_MODIFY)
...
#define UNARY_P(X)   \
  (GET_RTX_CLASS_EQ_RTX_UNARY(GET_CODE (X)))
...
#define COMMUTATIVE_ARITH_P(X)   \
  (GET_RTX_CLASS_EQ_RTX_COMM_ARITH(GET_CODE (X)))
...
#define CONSTANT_P(X)   \
  (GET_RTX_CLASS_EQ_RTX_CONST_OBJ(GET_CODE (X)))
...
/* commented out b/c our own RTL accessor */
#if 0
...
/* Commented because we define our own accessors here */
...
unsigned int XCUINT(const_rtx, int, enum rtx_code);
