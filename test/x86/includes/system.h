...
/* A macro to determine whether a VALUE lies inclusively within a
   ...
   negative.  */
/* Original definition */
#define ORIG_IN_RANGE(VALUE, LOWER, UPPER) \
  ((unsigned HOST_WIDE_INT) (VALUE) - (unsigned HOST_WIDE_INT) (LOWER) \
   <= (unsigned HOST_WIDE_INT) (UPPER) - (unsigned HOST_WIDE_INT) (LOWER))

/* My definition of IN_RANGE to expose >=0 check. Thought, unlike
   the original definition, we evalue VALUE twice. This can be a problem
   but I do not see how can we evaluate value once and still get what we
   want. */
#define IN_RANGE(VALUE, LOWER, UPPER) \
  (((unsigned HOST_WIDE_INT) (VALUE) >= (unsigned HOST_WIDE_INT) (LOWER)) && \
  ((unsigned HOST_WIDE_INT) (VALUE) <= (unsigned HOST_WIDE_INT) (UPPER)))
